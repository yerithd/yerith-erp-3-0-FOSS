                                                          ...
                                                          bool 				show_ASSET /* = false */)
{
    ...
    _currentStockID = YerithWindowsCommons::get_last_lister_selected_row_db_ID();

    if (!YerithUtils::isEqualCaseInsensitive(_previousStockID, _currentStockID))
    {
        clear_CURRENT_STOCK_HISTORY();
    }
    ...
    if (!_show_ASSET)
    {
        preambleTitle.append(QObject::tr(" ID (%1), stock \"%2\"")
        						.arg(YerithWindowsCommons::get_last_lister_selected_row_db_ID(),
        							 stockDesignation));
    }
    else
    {
...
void YerithHistoriqueDuStockWindow::SLOT_line_per_pagte_MODIFIED(const QString &
                                                                 a_line_per_page_NR)
{
...
{
    lineEdit_nombre_de_lignes_par_page->setValidator(&YerithUtils::UINT_VALIDATOR_GREATER_THAN_ZERO);
