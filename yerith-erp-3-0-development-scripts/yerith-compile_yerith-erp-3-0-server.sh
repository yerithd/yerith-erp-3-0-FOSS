#!/bin/bash
# @file: yerith-compile_yerith-erp-3-0-server.sh
# @author: Dr.-Ing. XAVIER NOUNDOU
# @email: YERITH.D@GMAIL.COM


if [[ "${@}" == *"s"* || "${@}" == *"h"* ]]; then
		yerith-compile_yerith-erp-3-0.sh -v YERITH_SERVER "${@}"
		exit
fi

if [[ "${@}" == *"e"* ]]; then
	OUT_CMD_TIME=$(time yerith-compile_yerith-erp-3-0.sh -v YERITH_SERVER "${@}" > /dev/null 2>&1) && \
